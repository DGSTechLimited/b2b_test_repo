...
async function seedCatalog() {
  ...
  const buildPart = (partType: PartType, index: number) => {
    ...
    const brand =
      partType === "AFTERMARKET" ? randomChoice(brands) : partType === "BRANDED" ? "Hotbray" : "OEM";
  ...
  for (let idx = 0; idx < 150; idx += 1) {
    ...
    if (lineStatuses.length > 0) {
      ...
      await prisma.orderLineStatus.createMany({
        data: Array.from(uniqueStatuses.values()).map((row) => ({
          ...row,
...
export async function disconnectDemoSeedDb() {
