...
export async function createDealerDb(
  formData: FormData,
...
export async function uploadPartsDb(
  formData: FormData,
  ...
  const batch = await createBatch(uploadType, file.name, userId);
  ...
  stagingRows.forEach((row) => {
    ...
    const category = isXlsx ? discountCodeToCategory(rawCategory) ?? "" : rawCategory.toUpperCase();
...
export async function uploadOrderStatusDb(formData: FormData, userId: string) {
  const file = formData.get("file") as File | null;
  if (!file) {
    throw new Error("CSV file is required.");
  }
  ...
  const batch = await createBatch("ORDER_STATUS", file.name, userId);
  const stagingRows = rows.map((row, index) => ({
    batchId: batch.id,
    rowNumber: index + 1,
...
export async function uploadSupersessionDb(formData: FormData, userId: string) {
  const file = formData.get("file") as File | null;
  if (!file) {
    throw new Error("CSV file is required.");
  }
  ...
  const batch = await createBatch("SUPERSESSION", file.name, userId);
  const stagingRows = rows.map((row, index) => ({
    batchId: batch.id,
    rowNumber: index + 1,
